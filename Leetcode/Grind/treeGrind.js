...
function buildTree(preorder, inorder) {
    ...
    function buildTreeHelper(inorderLo, inorderHi) {
        if (preorderIndex === preorder.length || inorderLo < 0 || inorderHi === inorder.length || inorderLo > inorderHi) {
            return null;
        }
...
function findWords(board, words) {
    ...
    function findWordsHelper(board, i, j, str, foundWords, dictionary) {
        ...
        findWordsHelper(board, i + 1, j, newStr, foundWords, dictionary);
        findWordsHelper(board, i - 1, j, newStr, foundWords, dictionary);
        findWordsHelper(board, i, j + 1, newStr, foundWords, dictionary);
        findWordsHelper(board, i, j - 1, newStr, foundWords, dictionary);

        board[i][j] = temp;

        function isInvalid(board, i , j, str, dictionary) {
            return i < 0 || i === board.length || j < 0 || j === board[0].length || board[i][j] === "@" || !dictionary.startsWith(str);
        }   
    }
